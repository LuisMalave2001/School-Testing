...
class SchoolBaseSettings(models.TransientModel):
    ...
    def _get_name_sorting_setting_ids(self):
        ...
        name_sorting_ids = self.env.ref("school.name_sorting_first_name") + \
                           self.env.ref("school.name_sorting_middle_name") + \
                           self.env.ref("school.name_sorting_last_name")

        return name_sorting_ids.sorted()

    name_sorting_setting_ids = fields.Many2many("school.name_sorting", string="Name Sorting",
    ...
    # current_school_year_id = fields.Many2one('school.school.year', related='company_id.current_school_year_id', readonly=False)
    # enrollment_school_year_id = fields.Many2one('school.school.year', related='company_id.enrollment_school_year_id', readonly=False)

    @api.onchange("name_sorting_setting_ids")
    def _onchange_name_sorting_setting_ids(self):
        name_order_relation = {self.env.ref("school.name_sorting_first_name").id: _("First"),
                               self.env.ref("school.name_sorting_middle_name").id: _("Middle"),
                               self.env.ref("school.name_sorting_last_name").id: _("Last")}
