...
class SchoolBaseHomeAddress(models.Model):
    ...
    def name_get(self):
        names_list = super(SchoolBaseHomeAddress, self).name_get()

        if self._context.get('lineal_name', False):
            aux_name_list = []
            for id, name in names_list:
                name = name.replace('\n', '; ').replace(' ; ', ' ')
                aux_name_list.append((id, name))
            names_list = aux_name_list
        return names_list
