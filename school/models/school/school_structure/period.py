...
class PeriodCategory(models.Model):
    ...
    def _compute_recursive_name(self):
        for period in self:
            period.name = period._get_recursive_parent_name()
...
class Period(models.Model):
    ...
    _order = 'parent_name, date_start, display_name'
    ...
    date_start = fields.Date(
        compute='compute_dates', store=True, readonly=False)
    ...
    def compute_dates(self):
        for period in self:
            if period.child_ids:
                first_child = period.child_ids.sorted('date_start')[0]
                last_child = period.child_ids.sorted('date_start', reverse=True)[0]
                period.date_start = first_child.date_start
                period.date_end = last_child.date_end
            else:
